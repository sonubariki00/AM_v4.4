...
def login():
    ...
    if user and user['password']==password:
        ...
        return jsonify({"message": f"{user['role'].capitalize()} logged in successfully", "user_id": user['id'], "role": user['role']}), 200
    else:
...
def request_asset():
    ...
    log_manager.add_log('INFO', 'asset_management', f"Asset request submitted for {asset_id} by {employee_id}", employee_id)
...
def release_asset():
    ...
    asset_id = request.args.get('asset_id')
    all_data = db.load_data()
    asset = next((a for a in all_data['tagged'] if a['asset_id'] == asset_id and a['employee_assigned'] == employee_id), None)
    if not asset:
        log_manager.add_log('ERROR', 'asset_management', f"Asset {asset_id} not found or not assigned to {employee_id}", employee_id)
...
def add_asset():
    ...
    if not asset_name or not asset_id:
        log_manager.add_log('ERROR', 'asset_management', f"Invalid asset details provided by admin {admin_id}", admin_id)
...
def remove_asset():
    ...
    if not asset:
        log_manager.add_log('ERROR', 'asset_management', f"Asset {asset_id} not found for removal by admin {admin_id}", admin_id)
        return jsonify({"message": "Asset not found"}), 404

    current_count = int(asset['count'])
    if current_count < remove_count:
        log_manager.add_log('ERROR', 'asset_management', f"Insufficient asset count for {asset_id} by admin {admin_id}", admin_id)
...
def add_employee():
    ...
    if not employee_name or not employee_id or not employee_password:
        log_manager.add_log('ERROR', 'user_management', f"Invalid employee details provided by admin {admin_id}", admin_id)
        return jsonify({"message": "Invalid employee details"}), 400

    all_data = db.load_data()
    if any(u['id'] == employee_id for u in all_data['users']):
        log_manager.add_log('ERROR', 'user_management', f"Employee ID {employee_id} already exists, attempted by admin {admin_id}", admin_id)
...
def remove_employee():
    ...
    if not employee:
        log_manager.add_log('ERROR', 'user_management', f"Employee {employee_id} not found for removal by admin {admin_id}", admin_id)
...
def analyze_logs():
    ...
    analysis_result = log_manager.analyze_logs(start_date, end_date)
